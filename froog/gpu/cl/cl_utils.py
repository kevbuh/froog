...
from .device_cl import OpenCLDevice, CL_AVAILABLE
...
def cl_pooling_krnl_build(cl_ctx: Any, iter_op: str, result_op: str, init_val: Union[int, str] = 0) -> Any:
    ...
    if device is None:
        return None
    if isinstance(device, OpenCLDevice):
        return device._build_pooling_kernel(iter_op, result_op, init_val)
    return None

def pooling_op(ctx: Any, input: Any, kernel_size: Tuple[int, int], 
